                    ...
                    await interaction.reply({
                        content:
                            config.messageSettings.developerMessage !== undefined &&
                                ...
                                ? config.messageSettings.developerMessage
                                : "You are not authorized to use this command nigga.",
                        ephemeral: true,
                    });
                    ...
                    await interaction.reply({
                        content:
                            config.messageSettings.missingDevIDsMessage !== undefined &&
                                ...
                                ? config.messageSettings.missingDevIDsMessage
                                : "This is a developer only command. Access Denied.",
                ...
                if (cooldown.has(cooldownKey)) {
                    ...
                    if (data.some((v) => v === interaction.commandName)) {
                        const cooldownMessage = isGlobalCooldown
                            ? config.messageSettings.globalCooldownMessage ?? "Slow down nigga. This command is on a global cooldown"
                            : config.messageSettings.cooldownMessage ?? "Slow down nigga. You're too fast to use this command";
